...
def get_gemini_response(input,content,prompt):  #input: how LLM model behave like, #image: To extract info, prompt: ask something
    # loading the gemini model
    model=genai.GenerativeModel('gemini-pro')
    response=model.generate_content([input,content,prompt]) #get response from model
...
if submit:
   # cont=read_pdf(uploaded_file)
    response=get_gemini_response(input_prompt,content,input)
